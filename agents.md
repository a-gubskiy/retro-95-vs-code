...
This repository contains Retro 95 VS Code Theme, a local Visual Studio Code color theme that mirrors the nearby Chromium browser theme in `../browser-theme`.

## Project Goal

Create a Visual Studio Code theme for the Retro 95 project that mimics the color language of classic 90s desktop applications without using Microsoft-owned logos, icons, screenshots, wallpapers, or official branding.
...
- `package.json` contributes the `Retro 95` color theme.
- `themes/retro-95-color-theme.json` contains the workbench colors, token colors, and semantic token colors.
...
- `themes/retro-95-color-theme.json` - the actual VS Code color theme.
...
The editor tabs intentionally use gray retro desktop-style tab controls rather than navy browser-style inactive tabs:
...
The command center / Copilot titlebar control is gray so black product icons remain readable, matching the black glyphs on gray classic title-bar buttons:
